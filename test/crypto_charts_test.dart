...
import 'package:currency_converter/src/core/rates/crypto/fawazahmed_crypto_usd_price_client.dart';
...
import 'package:currency_converter/src/features/charts/domain/chart_repository.dart';
...
import 'package:currency_converter/src/features/charts/presentation/chart_state.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:http/http.dart' as http;
import 'package:shared_preferences/shared_preferences.dart';

void main() {
  test('fawazahmed0 latest client rejects future-dated payloads', () async {
    final client = FawazahmedCryptoUsdPriceClient(
      client: _StaticHttpClient(
        jsonEncode(<String, Object>{
          'date': '2099-01-01',
          'usd': <String, Object>{'btc': 0.00001, 'eth': 0.0003},
        }),
      ),
    );
    ...
    await expectLater(
      ...
      throwsA(isA<CryptoUsdPriceException>()),
    );
  });
  ...
  test('charts controller downgrades 2Y to 1Y for crypto pair', () {
    final controller = ChartsController(
      ...
      repository: RatesServiceChartRepository(
        RatesService(client: _FakeRatesClient(), cache: _MemoryRatesCache()),
      ),
  ...
  test(
    'fawazahmed0 history client parses and inverts BTC/ETH from CDN',
    () async {
  ...
  test(
    'fawazahmed0 history client tolerates mixed success/failure per date',
    () async {
      final client = FawazahmedCryptoUsdHistoryClient(
  ...
  test(
    'multi-provider client composes BTC to ETH over a 5-day range',
    () async {
      SharedPreferences.setMockInitialValues(<String, Object>{});
      final prefs = await SharedPreferences.getInstance();
      final client = MultiProviderRatesClient(
  ...
  test(
    ...
    () async {
      ...
      expect(result.data[DateTime(2026, 5, 16)], closeTo(0.00002, 0.000000001));
      expect(
        result.data[DateTime(2026, 5, 17)],
        closeTo(1.20 / 61000, 0.0000001),
      );
    },
  );

  test('ChartsController.swapPair keeps 1Y range for crypto-involved pair', () {
  ...
  test('ChartsController.setRange downgrades 2Y to 1Y for crypto pair', () {
  ...
  test('ChartsController contains unexpected repository failures', () async {
    final controller = ChartsController(
      allowCryptoCharts: true,
      repository: _ThrowingChartRepository(),
    );
...
class _ThrowingChartRepository implements ChartRepository {
  @override
  Future<HistoricalResult> getHistoricalRates({
    ...
    throw StateError('unexpected provider failure');
  }
}
...
class _MixedSuccessHttpClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    if (url.contains('2024-03-06')) {
      return http.StreamedResponse(
        Stream.fromIterable([
          utf8.encode('{"date":"2024-03-06","usd":{"btc":0.000015192}}'),
        ]),
        200,
        request: request,
      );
